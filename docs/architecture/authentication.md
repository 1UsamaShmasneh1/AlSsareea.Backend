...
ASP.NET Core fixed-window policies cover login, customer registration, Google authentication, refresh, and OTP endpoints. A request-aware limiter after model binding adds per-operation partitions derived from normalized identifier/destination, token hash or challenge ID, IP, purpose, and body device identifier. Every partition key is SHA-256 hashed, and the outer middleware also partitions by IP, session, and `X-Device-Identifier`. Responses use 429 and `Retry-After`. Account lockout remains an independent defense with five failed attempts and a 15-minute default lock.

Customer registration is a dedicated anonymous operation that fixes `UserType.Customer` server-side, applies the existing normalized-email and password rules, and creates the device, session, access token, and rotating refresh token in the Identity boundary. The client uses a hashed idempotency key/fingerprint; a completed duplicate is rejected safely because raw refresh secrets are never persisted for response replay.

Google authentication uses the provider `sub` as the authoritative external key. Infrastructure validates the signed Google ID token with the official validator and additionally enforces configured audience, Google issuer, expiry, nonempty subject/email, `email_verified`, and nonce when supplied. An unlinked local-email collision returns `auth.external_link_required`; email alone never links accounts. External-only users persist a null password credential and therefore fail password login with the same public invalid-credential response. Production keeps Google disabled unless at least one allowed client ID is explicitly configured.
