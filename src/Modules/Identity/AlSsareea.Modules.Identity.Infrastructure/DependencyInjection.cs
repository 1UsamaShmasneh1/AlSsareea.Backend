...
public static class DependencyInjection
{
        ...
        IConfiguration configuration)
    {
        ...
        services.AddOptions<AuthenticationRateLimitOptions>().Bind(configuration.GetSection(AuthenticationRateLimitOptions.SectionName)).Validate(x => x.LoginPermitLimit > 0 && x.RefreshPermitLimit > 0 && x.OtpPermitLimit > 0 && x.RegistrationPermitLimit > 0 && x.GooglePermitLimit > 0 && x.WindowSeconds is >= 10 and <= 3600, "Authentication rate limits are invalid.").ValidateOnStart();
        services.AddOptions<GoogleAuthenticationOptions>().Bind(configuration.GetSection(GoogleAuthenticationOptions.SectionName)).Validate(x => !x.Enabled || x.AllowedClientIds.Length > 0 && x.AllowedClientIds.All(id => !string.IsNullOrWhiteSpace(id)), "Google authentication must have at least one allowed client ID when enabled.").ValidateOnStart();
