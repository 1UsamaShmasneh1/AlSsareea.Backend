...
{
    private RefreshToken(RefreshTokenId id, UserId userId, DeviceId? deviceId, LoginSessionId loginSessionId, RefreshTokenHash tokenHash, Guid securityStampSnapshot, DateTime createdUtc, DateTime expiresUtc, IPAddress? createdByIpAddress, Guid? familyId) : base(id)
    {
        UserId = userId; DeviceId = deviceId; LoginSessionId = loginSessionId; TokenHash = tokenHash; SecurityStampSnapshot = securityStampSnapshot; CreatedUtc = createdUtc; ExpiresUtc = expiresUtc; CreatedByIpAddress = createdByIpAddress; FamilyId = familyId;
    }
    ...
    public DateTime? ReplayDetectedUtc { get; private set; }
    public bool IsActive(DateTime utcNow) => ConsumedUtc is null && RevokedUtc is null && utcNow < ExpiresUtc;
    public static RefreshToken Create(RefreshTokenId id, UserId userId, DeviceId? deviceId, LoginSessionId sessionId, RefreshTokenHash hash, Guid securityStampSnapshot, DateTime createdUtc, DateTime expiresUtc, IPAddress? createdByIpAddress = null, Guid? familyId = null)
    {
        ...
        return new RefreshToken(id, userId, deviceId, sessionId, hash, securityStampSnapshot, createdUtc, expiresUtc, createdByIpAddress, familyId);
    }
    ...
    public void MarkReplayDetected(DateTime occurredUtc) { DomainRules.RequireUtc(occurredUtc, nameof(occurredUtc)); ReplayDetectedUtc = occurredUtc; }
