...
public static class DependencyInjection
{
    public static IServiceCollection AddDispatchingInfrastructure(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        services.AddOptions<DispatchingOptions>().Bind(configuration.GetSection(DispatchingOptions.SectionName)); services.AddDbContext<DispatchingDbContext>(options => options.UseNpgsql(connection, n => n.MigrationsAssembly(typeof(DispatchingDbContext).Assembly.FullName).MigrationsHistoryTable(DispatchingPersistence.MigrationsHistoryTable, DispatchingPersistence.Schema)).UseSnakeCaseNamingConvention()); services.AddHealthChecks().AddDbContextCheck<DispatchingDbContext>("dispatching-postgresql", tags: ["ready"]); services.AddScoped<IIntegrationEventSource, DispatchingOutboxSource>(); services.AddScoped<IDispatchRepository, DispatchRepository>(); services.AddScoped<IDispatchService, DispatchService>(); services.AddHostedService<DispatchExpirationWorker>(); return services;
    }
}
