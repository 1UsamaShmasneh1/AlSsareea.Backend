...
internal sealed class CatalogService(CatalogDbContext db, ICatalogRepository catalogs, IProductRepository products, IMerchantCatalogScopeProvider merchants, IMediaAssetLookup media, IClock clock) : ICatalogService, IProductSnapshotProvider, ICartCatalogValidationService
{
    ...
    public async Task<ProductSnapshot?> BuildAsync(Guid merchantId, Guid productId, Guid? variantId, IReadOnlyList<Guid> optionIds, string language, CancellationToken ct = default) { CatalogOperationResult<CatalogPriceResponse> price = await CalculatePriceAsync(merchantId, productId, new PriceRequest(variantId, optionIds, language), ct); if (price.Value is null) return null; Product x = (await products.GetAsync(merchantId, new ProductId(productId), false, ct))!; ProductTranslation text = x.Translations.FirstOrDefault(t => t.LanguageCode == language) ?? x.Translations.First(); Dictionary<Guid, OptionGroup> groups = x.OptionGroups.ToDictionary(g => g.Id.Value); return new ProductSnapshot(productId, x.CurrentVersion, merchantId, x.CatalogId.Value, text.Name, x.Sku, x.BasePriceMinor, x.Currency, variantId, price.Value.SelectedVariant?.Name, price.Value.VariantAdjustmentMinor, price.Value.SelectedOptions.Select(o => { ProductOption po = x.OptionGroups.SelectMany(g => g.Options).Single(v => v.Id.Value == o.Id); OptionGroup g = groups[po.OptionGroupId.Value]; return new SnapshotOption(g.Id.Value, g.Translations.First().Name, o.Id, o.Name, o.AdjustmentMinor); }).ToArray(), x.TaxCategoryReference, price.Value.TotalPriceMinor, clock.UtcNow); }
